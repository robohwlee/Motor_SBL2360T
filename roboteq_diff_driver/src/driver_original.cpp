...
ROS_DEBUG_STREAM("cmdvel rpm right: " << right_rpm << " left: " << left_rpm);
  ...
  // int32_t right_encoder = getvalue(_S, 01);
  // int32_t left_encoder = getvalue(_S,1);
  

  // Ref. read function in the code
    // controller.write("# C_?HS 1_# 50\r");
  ...
  // controller.write("?HS 1");
  // controller.flush();


  // char read_buffer = 0;
  // int hs_vel_buff = controller.read(&read_buffer, 1);
  // int hs_vel_buff = controller.read((char*)&read_buffer, 10);
  ...
  // std::string hs_vel = controller.read();
  // std::cout<<"Hall snesor value is "<<hs_vel<<"\r";
  // uint8_t hs_vel_int = controller.read();
  // std::cout<<"Hall snesor value is "<<hs_vel_int<<"\r";
}
